...
public class ProductController {
    ...
    public ResponseEntity<?> getAll(Pageable pageable){
        // 페이지 번호를 number로 주시면 안됨! page 로 전달해 주어야 한다.
        // 사용자가 선택한 페이지 번호 -1을 클라이언트 단에서 해서 전달해 주어야 한다
        List<ProductResDto> dtoList = productService.productList(pageable);
        log.info("product: {}", dtoList);

        CommonResDto resDto = new CommonResDto(HttpStatus.OK, "상품 리스트 조회 성공", dtoList);
